...
class toba_db
{
	...
	*/
	function conectar()
	{
		if(!isset($this->conexion)) {
			try {
				...
				$this->log("No es posible realizar la conexi�n a la base. Mensaje: " . $e->getMessage() , 'error');
	...
	/**
	 * Activa/desactiva el resgistro de los comandos de ejecuci�n.
	...
	/**
	 * Exporta el registro de los comandos de ejecuci�n y consulta a archivos.
	 * @param string $archivo_ejecucion Archivo al que se desea exportar el contenido del registro de ejecuci�n.
	...
	function exportar_registro($archivo_ejecucion = '', $archivo_consulta = '')
	{
		try {
			...
			if ($this->registrar_ejecucion)
			{
				...
				if ($pos === false) {
					$ee = new Exception('Ruta del archivo de ejecuci�n inv�lida');
					throw $ee;
				}
				
				if (!file_exists(substr($archivo_ejecucion, 0, $pos))) {
					$ee = new Exception('Ruta del archivo de ejecuci�n inexistente');
					throw $ee;
				}
			}
			...
			if ($this->registrar_consultas)
			{
				...
				if ($pos === false) {
					$ee = new Exception('Ruta del archivo de consulta inv�lida');
					throw $ee;
				}
	...
	*/
	...
	/**
	 * Devuelve el id del �ltimo insert en la base. Si el motor es postgres
