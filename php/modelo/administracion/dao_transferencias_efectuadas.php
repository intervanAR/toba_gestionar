...
/**
 * Description of dao_transferencias_efectuadas
 *
 * @author fbohn
 */
class dao_transferencias_efectuadas {
	
	static public function get_transferencias_efectuadas($filtro= array())
	{        
        $where= "1=1";
        $where .= " AND " . ctr_construir_sentencias::get_where_filtro($filtro, 'tr', '1=1');
        
        $sql = "SELECT tr.*
                FROM AD_TRANSFERENCIAS_EFECTUADAS tr
                WHERE  $where";
    ...
    static public function get_transferencia_efectuadas_x_id($id_comprobante_pago) 
	{
	...
	static public function get_lov_transferencias_efectuadas_x_nombre($nombre, $filtro = array()) 
	{
		if (isset($nombre)) {
	...
	static public function get_datos_transferencia_efectuadas_x_id($id_comprobante_pago) 
	{
        if (isset($id_comprobante_pago)) {
            $sql = "SELECT	tr.*,   
							tr.ID_COMPROBANTE_PAGO||' - '||tr.nro_transferencia||' - '||to_char(tr.fecha_transferencia, 'dd/mm/rr')||' - '||tr.IMPORTE as lov_descripcion,
							tr.nro_transferencia nro,
							to_char(tr.FECHA_transferencia, 'dd/mm/rrrr') fecha_format,
							tr.FECHA_transferencia fecha
                    FROM AD_TRANSFERENCIAS_efectuadas tr
                    WHERE tr.id_comprobante_pago = ".quote($id_comprobante_pago) .";";
