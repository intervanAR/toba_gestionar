...
/**
 * Description of dao_retenciones_recibidas
 *
 * @author hmargiotta
 */
class dao_retenciones_recibidas {
	static public function get_retenciones_recibidas($filtro= array()){        
        $where= "1=1";
        $where .= " AND " . ctr_construir_sentencias::get_where_filtro($filtro, 'rr', '1=1');
        
        $sql = "SELECT rr.*
                FROM AD_RETENCIONES_RECIBIDAS rr
                WHERE  $where";
    ...
    static public function get_retencion_recibida_x_id($id_comprobante_pago) {
        if (isset($id_comprobante_pago)) {
            $sql = "SELECT rr.*,   rr.ID_COMPROBANTE_PAGO||' - '||rr.nro_comprobante||' - '||to_char(rr.fecha_retencion, 'dd/mm/rr')||' - '||rr.IMPORTE as lov_descripcion
                    FROM AD_RETENCIONES_RECIBIDAS rr
                    WHERE rr.id_comprobante_pago = ".quote($id_comprobante_pago) .";";
	...
	static public function get_lov_retenciones_recibidas_x_nombre($nombre, $filtro = array()) {
		if (isset($nombre)) {
        ...
        $sql = "SELECT  rr.*, 
                        rr.ID_COMPROBANTE_PAGO||' - '||rr.nro_comprobante||' - '||to_char(rr.fecha_retencion, 'dd/mm/rr')||' - '||rr.IMPORTE as lov_descripcion
                FROM AD_RETENCIONES_RECIBIDAS rr
                WHERE $where
