 ...
 * @author ddiluca
 */
class toba_vista_jasperreport_intervan
{
...
//	protected $path_plantilla = '';
    ...
    public function generar_salida()
    {
        foreach ($this->objetos as $objeto) {
            if (method_exists($objeto, 'vista_jasperreports_intervan')) {
                $objeto->vista_jasperreports_intervan($this);
            }
        }
    ...
    */
    ...
    protected function configurar_bd(&$conexion)
    {
        ...
        $port = (isset($params['puerto'])) ? ':'.$params['puerto'] : '';
        //$con->setConnectString("jdbc:postgresql://".$params['profile'].$port.'/'.$params['base']);
        $con->setConnectString('jdbc:oracle:thin:@(DESCRIPTION=(ADDRESS=(HOST=10.1.1.1)(PROTOCOL=tcp)(PORT=1521))(CONNECT_DATA=(SERVICE_NAME=TEST10G)))');
        //Especificamos los datos de la conexión, cabe aclarar que esta conexion es la del servidor de producción
        echo 'PASAAAAAAAAAAAAAAAAAAAAA';
        $con->setUser('ren_riocol');
        $con->setPassword('ren_riocol');
        $con1 = $con->getConnection();
        /*
        if (isset($params['schema'])) {
            $sql = "SET search_path = \"{$params['schema']}\", \"public\";";
