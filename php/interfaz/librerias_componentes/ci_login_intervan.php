...
class ci_login_intervan extends toba_ci
{
    protected $s__datos;
    ...
    public function ini()
    {
        if (toba::instalacion()->get_tipo_autenticacion() == 'saml') {
            toba::manejador_sesiones()->set_autenticacion(new toba_autenticacion_saml_intervan());
        }

        if (toba::instalacion()->get_tipo_autenticacion() == 'ldap') {
            toba::manejador_sesiones()->set_autenticacion(new toba_autenticacion_ldap_intervan());
        }
        ...
        if (in_array($tipo_auth, ['cas', 'saml'])) {
    ...
    public function post_eventos()
    {
        ...
        try {
            ...
            $this->resetear_marca_login();
            $tipo_auth = toba::instalacion()->get_tipo_autenticacion();
            if (in_array($tipo_auth, ['cas'])) {
                $usuario = toba::manejador_sesiones()->get_autenticacion()->get_usuario_toba();
            } else {
                $usuario = $this->s__datos['usuario'];
            }
            toba::vinculador()->navegar_a('principal', '12000119', ['forzar_cambio' => '1', 'usuario' => $usuario]);
    ...
    public function invocar_autenticacion_por_tipo()
    {
        ...
        } elseif (in_array($tipo_auth, ['cas', 'saml']) && toba::manejador_sesiones()->get_autenticacion()->uso_login_centralizado()) {    //El control por session es para que no redireccione automaticamente
            toba::manejador_sesiones()->get_autenticacion()->verificar_acceso();
        }
    }
    ...
    {
        if (toba::memoria()->get_dato_instancia('toba_intentos_fallidos_login') === null) {
            $form->desactivar_efs(['test_error_repetido']);
        }
        if (toba::instalacion()->get_tipo_autenticacion() != 'openid') {
            $form->set_titulo('');
        }
    ...
    public function conf__openid(toba_ei_formulario $form)
    {
        ...
        if (!empty($providers)) {
            ...
            $form->set_datos_defecto(['provider' => $provider['id']]);
        }
        if (isset($this->s__datos_openid)) {
    ...
    public function evt__cas__ingresar()
    {
        ...
        try {
            $logout = toba::memoria()->get_parametro('logout');
            if (isset($logout) && $logout == '1') {
                toba::manejador_sesiones()->get_autenticacion()->logout();
            } else {
                toba::manejador_sesiones()->get_autenticacion()->verificar_acceso();
            }
        } catch (toba_error_login_contrasenia_vencida $e) {
