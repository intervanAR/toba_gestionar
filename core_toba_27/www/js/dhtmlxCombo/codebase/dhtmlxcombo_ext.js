...
dhtmlXCombo.prototype.getOptionByLabel = function(value) {
    /*value=value.replace(/&/g,"&amp;") se comenta porque no funciona con descripciones en combos editables que contengan el caracter & */
    for (var i = 0; i < this.optionsArr.length; i++)
...
dhtmlXCombo.prototype.getPosition = function(oNode, pNode) {
	...
	var r= oCurrentNode.getBoundingClientRect();
	var iLeft=r.left;
	var iTop=r.top+window.scrollY;;
	/*
...
dhtmlXCombo.prototype._onBlur = function() {
	...
	if (self.DOMlist.childNodes.length == 1 && self.getComboText() != '') {
		var texto = self.getSelectedText();
		if (texto == '' || texto != self.getComboText()) {
			self.selectOption(0,false, true);
		}
	}
};
...
dhtmlXCombo.prototype.loadXML = function(url, afterCall) {
    ...
    if ((this._xmlCache) && (this._xmlCache[url])) {
        ...
        if (this._prs)
            for (var i = 0; i < this._prs.length; i++) url += [getUrlSymbol(url), escape(this._prs[i][0]), "=", escape(this._prs[i][1])].join("");
        xml._cPath = url;
        xml.loadXML(url)
    }
};
...
dhtmlXCombo_defaultOption.prototype.select = function() {
    if (this.content) this.content.className = "dhx_selected_option"
};
...
toba.confirmar_inclusion('dhtmlxCombo/codebase/dhtmlxcombo');