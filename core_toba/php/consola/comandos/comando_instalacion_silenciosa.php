	...
	*/
	protected function definir_id_grupo_desarrollo($param)
	{
		...
		if ($es_invalido) {									//Si aun sigue faltando el parametro, defaulteo.
			toba::logger()->error('Se selecciono 0 cero como id de desarrollador, ya que uno v?lido no fue provisto');
			return 0;			
		}		
	...
	protected function definir_tipo_instalacion_produccion($param)
	{
		...
		$result = $this->recuperar_dato_y_validez($param, $nombre_parametro);
		if ($result['invalido']) {
			toba::logger()->error('Se selecciono tipo de instalaci?n de producci?n, ya que el tipo no fue provisto');
			return 1;
		}
	...
	protected function definir_nombre_instalacion($param)
	{
		...
		$result = $this->recuperar_dato_y_validez($param, $nombre_parametro);		
		if ($result['invalido']) {
			toba::logger()->error("Se selecciono el texto 'Sin nombre'  como nombre para la instalaci?n, ya que uno v?lido no fue provisto");
			return 'Sin nombre';
		}		
		return $result['resultado'];
	}
	...
	protected function definir_profile_motor($param)
	{
		...
		$result = $this->recuperar_dato_y_validez($param, $nombre_parametro);		
		if ($result['invalido']) {
			toba::logger()->error("Se selecciono 127.0.0.1 como ubicaci?n por defecto, ya que una v?lida no fue provista");
			return '127.0.0.1';
		}		
		return $result['resultado'];
	}
	...
	protected function definir_usuario_motor($param)
	{
		...
		$result = $this->recuperar_dato_y_validez($param, $nombre_parametro);		
		if ($result['invalido']) {
			toba::logger()->error("Se selecciono postgres como nombre de usuario por defecto, ya que uno v?lido no fue provisto");
			return 'postgres';
		}		
		return $result['resultado'];
	}	
	...
	protected function definir_base_motor($param)
	{
		...
		$result = $this->recuperar_dato_y_validez($param, $nombre_parametro);		
		if ($result['invalido']) {
			toba::logger()->error("Se selecciono '$nombre_toba' como nombre de bd, ya que uno v?lido no fue provisto");
			return $nombre_toba;
		}		
		return $result['resultado'];
	}
	...
	protected function definir_puerto_motor($param)
	{
		...
		$result = $this->recuperar_dato_y_validez($param, $nombre_parametro);		
		if ($result['invalido']) {
			toba::logger()->error("Se selecciono el puerto 5432 por defecto, ya que uno v?lido no fue provisto");
			return '5432';
		}		
		return $result['resultado'];
	}	
	...
	protected function definir_clave_motor($param)
	{
		...
		if ($es_invalido) {
			toba::logger()->error("Se selecciono postgres como clave por defecto, ya que una v?lida no fue provista");
			return 'postgres';
		}		
		return $clave;
	}	
